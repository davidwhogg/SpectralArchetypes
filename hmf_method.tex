...
% - do we need to say that the specific components don't matter as much as the space they span?
% - make sure we cite the relevant robusty-PCA methods (Wild, Budavari, others)

\documentclass[12pt,preprint,pdftex]{aastex}
\usepackage{amssymb,amsmath,mathrsfs}

\slugcomment{\texttt{draft 2011-09-08 / not ready for distribution}}
...
determination (\citealp{budavari}; \citealp{glazebrook}), emission-line measurements and properties of objects (\citealp{allen}; 
\citealp{borosona}; \citealp{borosonc}; \citealp{wild}), 
decomposition into different stellar populations (\citealp{chen}; 
\citealp{ferreras}; \citealp{nolan}) and outlier indedification and classification of sources (\citealp{borosona}; \citealp{connolly}; \citealp{francis}; \citealp{suzuki}; \citealp{yip}; \citealp{yipb}) can all in
...
components analysis (PCA) or similar approaches (HOGG CITE SDSS).
...
Cite Roweis review.  Mention also Robust PCA and outliers.  Also
mention Wild type things that patch PCA heuristically; criticise them.
...
most important outliers found in the \sdss\ data (HOGG CITE) are
...
(HOGG CITE) or some similar kind of train-and-test framework.  In this
...
(HOGG: Should we also mention that the $g_{k[j-1]}$ 
...
before performing PCA.  The initialization we use is the mean spectrum
...
the $\vec{f}_i$ live) so that the median [VIVI: MEDIAN?] of the $N$
...
In equations, this conditioning, mean determination, and projection to
the orthogonal space look like this: [HOGG: EQUATIONS HERE]
...
for Main Galaxies and LRGs and 10000 for QSOs, the numbers were selected as
such in order to make sure that we have at least as many objects as
number of pixels, which was needed for some of the tests we performed
using PCA). The spectra were taken from the 7th Data Release (DR7) of
SDSS (\textbf{ref}). The values of redshift were selected to be in the
ranges of 0.01-0.06, 0.20-0.50 and 0.10-1.50 for Main Galaxies, LRGs and
QSOs respectively.
...
and CUT II for LRGs with z$>$0.4. These criteria are suitable for
...
from 0.2 to 0.5 (95,833 sources). To do that we used the 
primTarget field that is provided in the SpecObjAll table of SDSS, which is
set to GALAXY\_RED if the LRG passes either CUT I or CUTII and to
GALAXY\_RED\_II if it passes CUTII but not CUTI. We
...
GALAXY\_RED as well) and made sure that they meet the criteria of \cite{eisenstein}.

The sample was used to determine the maximum wavelength coverage,
i.e. a wavelength area for which at least 10 sources have valid data
at the beginning and the end of the spectrum. In this way we are able
to fit the part of the spectrum that is produced by the second object
for a large range of redshift values. During this procedure, pixels
with any of the flags: SP\_MASK\_FULLREJECT, SP\_MASK\_NODATA,
SP\_MASK\_BRIGHTSKY, SP\_MASK\_NOSKY or pixels that correspond to zero
noise were treated as masked. All the spectra and their noise were
...
wavelength coverage for each object is: 3580.964-9109.615 \AA\ for
Main Galaxies, 2544.486-7615.528 \AA\ for LRGs and 1522.299-8352.183
\AA\ for QSOs, corresponding to 4056, 4762 and 7394 pixels
respectively. Since only 10 spectra include data at the bluest and
reddest wavelengths, missing data is present in almost all the spectra
of our sample. To deal with this problem we have set the fluxes at
those areas equal to the first or the last non-masked pixel of the
spectrum and we have set the noise of those pixels to a very high
value ($10^{-12} erg/sec/cm^2/$\AA), so that they will not be taken
into account by the method. As a last step before the application of
the method all spectra were interpolated to the wavelengths selected
for each type of source using cubic splines. The selected wavelengths
are uniformly distributed in log space as in the case of the original
SDSS wavelengths. For the resulting spectra we interpolated linearly
...
order to detect the second redshifts in the SLACS and BHB candidate
samples, we have decided to fit the SDSS spectra using the 14, 7 and
14 components that were produced by our method for Main Galaxy, LRG
and QSO sources after 16 iterations and using $\epsilon$=3, 30 and 10
respectively. It is interesting that fewer components are needed in
order to fit well the LRG SDSS spectra than are needed for the Main
...
spectra we test the improvement of the fitting when a spectrum is
fitted with two sets of componets at two different redshifts (i.e. the
...
logarithmic scale, i.e. in the same way as the wavelengths in SDSS. In
...
$z_n$ respectively). By definition an improvement of the fitting when
using an additional set of components will result to a positive value
of $\Delta\chi_{in}^2$. In the case that the spectrum has significant
flux coming from a second object we expect that there will be a peak
in $\Delta\chi_{in}^2$ at a second redshift equal to the one of that
object. The strength of the peak depends on the brightness and
therefore the distance of the second object, the presence of emission
lines in its spectrum and to some extent to the spectral coverage. As
it is obvious the use of an additional set of componets always
improves the fitting of the spectrum. However, this improvement is not
significant and does not vary a lot for different values of the second
redshift in the cases that the signature of a second object is not
present in the spectrum. Finally, by performing our search using the
difference of the $\chi^2$ values between the two fits, values of
...
we try to apply the test described above
(\sectionname~\ref{sec:tworedshifts}) and reproduce the results of the
...
the proceedure described in (\sectionname~\ref{sec:tworedshifts}) to
search for peaks of the $\Delta\,\chi_{in}^2$ in the second redshift. 
For the case that both sets of components used correspond to Main Galaxy 
spectra, we detected peaks for 120 SLACS spectra at the same redshifts 
as those found in the SLACS survey (\figurename~\ref{fig:6}). An additional 
criterion that we used was that the peaks (if present) should 
correspond to fits that did not produce negative OIII lines if 
they were included in the spectral range of the fit.
...
probability according to our method, and a set of components (Main Galaxy)
...
been selected to be BHB candidates\footnote{a new candidate by 
\cite{barrows11} appeared in the literature only after the complition of this work.} 
...
type of object (\citealp{tsalmantza}). The search resulted to 32
...
with SDSS Main Galaxy and LRG spectra, to fit the candidates. The
...
object. Similar were the results for the case of the four known BHBs
...
engineering and statistics literature.  We also thank Coryn
Bailer-Jones (MPIA), Mike Blanton (NYU), Jo Bovy (NYU), Adam Bolton
(Utah), Roberto Decarli (MPIA), Massimo Dotti (MPA), Dustin Lang
(Princeton), Hans-Walter Rix (MPIA), Kester Smith (MPIA), and Ben
Weaver (NYU) for valuable comments and discussions.  Partial funding
for this project was provided by NASA (grant NNX08AJ48G), the NSF
(grant AST-0908357), and the Alexander von Humboldt Foundation.  This
research made use of the SAO/NASA \project{Astrophysics Data System}
and the \project{R} Project for Statistical Computing.  All the code
used in this project is available from the authors upon request.
...
\clearpage
\begin{figure}
\includegraphics[angle=-90,width=0.49\columnwidth]{paper_plots/1qq}
\includegraphics[angle=-90,width=0.49\columnwidth]{paper_plots/2qq}
\includegraphics[angle=-90,width=0.49\columnwidth]{paper_plots/3qq}
...
\clearpage
\begin{figure}
\includegraphics[angle=-90,width=0.49\columnwidth]{paper_plots/1gg}
...
\caption{As \figurename~\ref{fig:6a} for the case of SLACS candidates
  that were fitted with two sets of Main Galaxy components.}
\label{fig:6}
\end{figure}

\clearpage
\begin{figure}
\includegraphics[angle=-90,width=0.49\columnwidth]{paper_plots/103lg}
...
\clearpage
\begin{figure}
\includegraphics[angle=-90,width=0.49\columnwidth]{paper_plots/12ggg}
...
\caption{As \figurename~\ref{fig:6a} for the case of SLACS candidates
  that were fitted with three sets of components (Main Galaxies and
...
\clearpage
\begin{figure}
\includegraphics[angle=-90,width=0.49\columnwidth]{paper_plots/36gg}
\includegraphics[angle=-90,width=0.49\columnwidth]{paper_plots/36ggSLACS}
\includegraphics[angle=-90,width=0.49\columnwidth]{paper_plots/66gg}
