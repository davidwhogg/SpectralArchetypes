...
minimum.  These steps must be modified when smoothness or
non-negativity regularizations are applied, as we discuss next.
...
HOGG Using \equationname~(\ref{eq:9}) we can minimize the $\chi^2$ value
...
\begin{eqnarray}\label{eq:astep-conj}\displaystyle
...
\begin{eqnarray}\label{eq:gstep-conj}\displaystyle
...
HOGG: NOTE THAT WE CAN'T DO ANY ROTATIONS (like SVD) IN THE SPACE WHEN
WE ARE NN, all we can do is re-order the spectra.
...
and Hans-Walter Rix (MPIA) for valuable discussions.  Partial funding
...
VIVI: GRANTS AND GRANT NUMBERS FOR YOU?
