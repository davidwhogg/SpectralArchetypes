...
% to-do
% -----
% - intro needs to make it clear that there is nothing special about spectra
...
\slugcomment{\texttt{draft 2010-12-26 / not ready for distribution}}
...
it is possible to approximate an exact solution by HOGG MODIFYING THE G-STEP HOW?

HOGG: DROP?  We can minimize the $\chi^2$ value for each wavelength J
and define the new set of basis functions. Since the estimation of the
component values at the Jth pixel requires their values at the (J+1)
and (J-1) pixels, we make use of the values that the components had in
those pixels at the previous iteration.

HOGG: Also, when the system becomes non-sparse, it might become
sensible to use conjugate gradient method \citep[for
...
THOUGHT WERE MISSING.)  In our limited tests in the \project{R}
language, we found that conjugate gradient was much faster per
iteration than the block-diagonal method but required many more
iterations to converge to the same precision.  We therefore used the
block-diagonal method in everything that follows.  But the relative
speed of one-shot optmization with a sparse matrix representation, the
block-diagonal versions, and the conjugate-gradient version will in
principle depend on the linear algebra system being used, quantitative
...
HOGG: NEEDS TO BE MASSAGED -- four initializations; PCA is best in one
case, K-means in the other.
...
and coefficients, we are forced to use an initialization with positive
values. Even though we could use any set of positive basis functions,
the best initialization seems to be the results of the K-means
algorithm. That is expected since the components extracted by K-means,
that correspond to the centers of the groups into which the algorithm
divides the training spectra, include more physical information than
other non-negative initializations. We expect that the K-means results
will include only positive values because if the number of groups is
not very large (this might result in groups with very few spectra as
...
VIVI: PEOPLE TO ACK?  GRANTS AND GRANT NUMBERS FOR YOU?
