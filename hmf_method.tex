...
a-step involves an iteration over objects $i$, and the g-step involves
an iteration over wavelength indices $j$.

The \emph{a-step} is, for each $i$,
\begin{eqnarray}\label{eq:astep}\displaystyle
\tv{A}_i & \leftarrow & \inverse{\tv{G}_i}\cdot\tv{F}_i \nonumber\\
...
The \emph{g-step} is, for each $j$,
\begin{eqnarray}\label{eq:gstep}\displaystyle
\tv{G}_j & \leftarrow & \inverse{\tv{A}_j}\cdot\tv{F}_j \nonumber\\
...
using conjugate gradient descent we update the coefficients based on
the equations
