...
of data points (set by the size of the data set), $M$ is the number of
wavelengths (set by the size of each data point), and $K$ is the
number of components permitted, or the dimensionality of the model
space.  The dimension $K$ has to be chosen either arbitrarily by the
investigator, or else by a process of model selection.

HOGG: quickly bash AIC, BIC, and Bayes integral; praise cross-validation.

HOGG: Note that epsilon is also a model complexity parameter; explain
why and praise cross-validation.
...
\acknowledgments It is with great regret that we thank Sam Roweis
(deceased), who initiated this work by suggesting the model and all
the optimization strategies.  It is a pleasure to thank Mike Blanton
(NYU), Jo Bovy (NYU), Adam Bolton (Utah), Roberto Decarli (MPIA),
Dustin Lang (Princeton), Iain Murray (Edinburgh), and Hans-Walter Rix
(MPIA) for valuable discussions.  Partial funding for this project was
