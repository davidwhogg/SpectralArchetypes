...
% to-do
% -----
% - DO WE TALK ABOUT CHOOSING K ANYWHERE BUT THE DISCUSSION?
...
\slugcomment{\texttt{draft 2011-11-16 / not ready for distribution}}
...
components analysis (PCA) or similar approaches (\eg, VIVI CITE SDSS DR7).
...
some similar kind of train-and-test framework.  In this method, the
...
(DR7) of SDSS (\textbf{ref}). The values of redshift were selected to
...
the beginning and the end of the spectrum. In this way we are able to
...
$2544.486<\labmda<7615.528\,\AA$ for LRGs and
$1522.299<\labmda<8352.183\,\AA$ for QSOs, corresponding to 4056, 4762
and 7394 pixels respectively.

VIVI: CHECK AND EDIT THIS PARAGRAPH AND THE NEXT: For each spectrum
$i$ , we obtain the official \SDSS\ pixel flux densities $f_{ij}$
...
artifacts, missing data is present in all the spectra of our
sample. To deal with this problem we have set the fluxes at those
areas equal to the first or the last non-masked pixel of the spectrum
and we have set the noise of those pixels to a very high value
...
different initializations (or different methods) shouldn't be peformed
...
narrow lines don't appear in the observed spectral domain, and for
defining template spectra for redshift estimation of objects that are
going to observed at low signal-to-noise in future surveys; by
