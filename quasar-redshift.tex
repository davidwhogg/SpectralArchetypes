...
$\chi^2_\ell$
\begin{eqnarray}\displaystyle
\chi^2_\ell &=& \sum_{j=1}^M \frac{1}{\sigma^2_{\ell m}}
 \,\left[f_{\ell m} - \sum_{k=1}^K a_{\ell k}\,g_{kJ(m)}\right]^2 \nonumber \\
J(m) &=& j - n
\quad,
\end{eqnarray}
where $n$ is the redshift index.  The goal is probabilistic inference,
which requires computed likelihoods.  Under the assumptions laid out
in this section, the scalar $\chi^2_\ell$ is related to the likelihood
$\like_\ell$ by
\begin{eqnarray}\displaystyle
\chi^2_\ell &\equiv& Q + 2\,\ln\like_\ell \nonumber\\
...
TBD:  Marginalization integral turns into exactly what sum?
